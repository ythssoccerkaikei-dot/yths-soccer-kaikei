...
const storage = {
  ...
  async set(key, value) {
    try {
      ...
      if (deleteError) {
        console.error(`Supabase delete error for ${key}:`, deleteError);
        return false;
      }
      ...
      // キャメルケースをスネークケースに変換
      const dataToInsert = value.map(item => this.camelToSnake(item));
      ...
      if (error) {
        console.error(`Supabase insert error for ${key}:`, error);
        return false;
      }
