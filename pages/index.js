...
import { Save, Plus, Edit2, Trash2, LogOut, Users, Calendar, MapPin, DollarSign, FileText, Activity, BarChart3, Settings, Eye, EyeOff, Clipboard, Check, Menu, X } from 'lucide-react';
import { supabase } from '../lib/supabase';

// ストレージヘルパー（Supabase使用）
const storage = {
  // テーブル名とキー名のマッピング
  tableMap: {
    'users': 'users',
    ...
    'incomes': 'incomes',
    'expenses': 'expenses',
    'membershipPayments': 'membership_payments',
  ...
  async set(key, value) {
    try {
      ...
      // キャメルケースをスネークケースに変換
      const dataToUpsert = value.map(item => {
        const snakeItem = this.camelToSnake(item);
        
        // created_atがない場合は削除（upsertで自動設定されるため）
        if (!snakeItem.created_at) {
          delete snakeItem.created_at;
        }
        
        // 空文字列をnullに変換（PostgreSQLの型エラー回避）
      ...
      // デバッグ：送信データを確認
      console.log(`Upserting to ${tableName}:`, dataToUpsert);
      ...
      if (error) {
        ...
        console.error('Failed data:', dataToUpsert);
        return false;
      }

      console.log(`Successfully upserted ${dataToUpsert.length} records to ${tableName}`);
      return true;
    } catch (error) {
...
// 初期データ（Supabaseでは既にテーブルとデフォルトユーザーが作成済み）
const initializeData = async () => {
  console.log('Supabaseからデータを読み込みます');
  // Supabaseでは初期化不要（テーブルとデフォルトユーザーはSQLで作成済み）
  return true;
};
...
export default function AccountingApp() {
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead style={{ background: '#f7fafc' }}>
              <tr>
                ...
                <th style={{ padding: '16px', textAlign: 'left', fontSize: '14px', fontWeight: '600', color: '#4a5568' }}>ステータス</th>
                <th style={{ padding: '16px', textAlign: 'right', fontSize: '14px', fontWeight: '600', color: '#4a5568' }}>操作</th>
              </tr>
            </thead>
            <tbody>
              {yearMembers.map(member => {
                ...
                const handleStatusChange = async (newStatus) => {
                  const updated = members.map(m => 
                    m.id === member.id 
                      ? { 
                          ...m, 
                          status: newStatus,
                          retirementDate: newStatus !== 'active' ? new Date().toISOString().split('T')[0] : null
                        }
                      : m
                  );
                  setMembers(updated);
                  await storage.set('members', updated);
                  showNotification(
                    newStatus === 'active' ? '在籍中に戻しました' :
                    newStatus === 'retired' ? '引退に設定しました' :
                    '退部に設定しました'
                  );
                };
                
                return (
                  <tr key={member.id} style={{ borderTop: '1px solid #e2e8f0', opacity: isActive ? 1 : 0.7 }}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '800px' }}>
                ...
                <tbody>
                  {yearMembers.map(member => {
                    const paidCount = getPaidMonthsCount(member.id);
                    const isActive = !member.status || member.status === 'active';
                    const retirementMonth = member.retirementDate ? member.retirementDate.slice(0, 7) : null;
                    
                    return (
                      <tr key={member.id} style={{ borderTop: '1px solid #e2e8f0', opacity: isActive ? 1 : 0.7 }}>
                        <td style={{ padding: '12px 16px', fontSize: '14px', color: '#1a202c', position: 'sticky', left: 0, background: 'white', zIndex: 1 }}>
                          {member.name}
                          {!isActive && (
                            <div style={{ fontSize: '11px', color: '#856404', marginTop: '2px' }}>
                              {member.status === 'retired' ? '引退' : '退部'} ({member.retirementDate})
                            </div>
                          )}
                        </td>
                        ...
                        {monthsList.map(month => {
                          ...
                          // 退部済みで退部月より後の月はチェックボックスを無効化
                          const isAfterRetirement = retirementMonth && month > retirementMonth;
                          
                          return (
                            <td key={month} style={{ padding: '8px', textAlign: 'center', background: isAfterRetirement ? '#f7fafc' : 'white' }}>
                              {isAfterRetirement ? (
  ...
  const CoachActivities = () => {
    ...
    const filteredActivities = filterMonth
      ? myActivities.filter(a => a.date && a.date.startsWith(filterMonth))
      : myActivities;
    
    // 合計金額の計算（filteredActivitiesを使用）
    const totalFee = filteredActivities.reduce((sum, a) => sum + (Number(a.totalFee) || 0), 0);
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '20px', marginBottom: '32px' }}>
          <div style={{
            ...
            <p style={{ fontSize: '32px', fontWeight: '700' }}>{filteredActivities.length}回</p>
            <p style={{ fontSize: '14px', opacity: 0.9, marginTop: '4px' }}>
              {filterMonth ? `${filterMonth.slice(5)}月分` : '全期間'}
            </p>
          </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: '12px' }}>
              <DollarSign size={24} />
              <span style={{ marginLeft: '8px', fontSize: '14px', opacity: 0.9 }}>報酬合計</span>
            </div>
        ...
        <div style={{ background: 'white', borderRadius: '12px', overflow: 'hidden', border: '1px solid #e2e8f0' }}>
                ...
                return (
                  <tr key={activity.id} style={{ borderTop: '1px solid #e2e8f0' }}>
                    ...
                    <td style={{ padding: '16px', textAlign: 'right', fontSize: '14px', color: '#1a202c' }}>
                      ¥{(Number(activity.coachFee) || 0).toLocaleString()}
                    </td>
                    <td style={{ padding: '16px', textAlign: 'right', fontSize: '14px', color: '#1a202c' }}>
                      ¥{(Number(activity.transportCost) || 0).toLocaleString()}
                    </td>
                    <td style={{ padding: '16px', textAlign: 'right', fontSize: '14px', color: '#1a202c' }}>
                      ¥{(Number(activity.etcFee) || 0).toLocaleString()}
                    </td>
                    <td style={{ padding: '16px', textAlign: 'right', fontSize: '14px', color: '#1a202c', fontWeight: '600' }}>
                      ¥{(Number(activity.totalFee) || 0).toLocaleString()}
                    </td>
          ...
          {filteredActivities.length === 0 && (
            <div style={{ padding: '40px', textAlign: 'center', color: '#718096' }}>
              {filterMonth ? '該当する活動記録がありません' : '活動記録がありません'}
            </div>
          )}
        </div>
      </div>
  ...
  const CoachPaymentManagement = () => {
    ...
    const coachSummaries = displayCoaches.map(coach => {
      ...
      const totalPayment = coachActivities.reduce((sum, a) => {
  ...
  const FiscalReport = () => {
    ...
    const yearMembers = members.filter(m => m.fiscalYearId === selectedYear.id);
    ...
    // 入金済み会費の合計を計算
    const totalMembershipFees = yearMembers.reduce((sum, member) => {
      // 各月の入金状況をチェック
      const paidCount = monthsList.filter(month => {
        const paymentKey = `${selectedYear.id}-${member.id}-${month}`;
        const payment = membershipPayments.find(p => p.id === paymentKey);
        return payment?.paid || false;
      }).length;
    ...
    const totalIncomeFromRecords = yearIncomes.reduce((sum, i) => sum + i.amount, 0);
    const totalIncome = totalIncomeFromRecords + totalMembershipFees; // 収入管理 + 入金済み会費
    ...
    if (totalMembershipFees > 0) {
      incomeByCategory['会費収入'] = totalMembershipFees;
    }
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px', marginBottom: '32px' }}>
          <div style={{
            ...
            <div style={{ marginTop: '12px', fontSize: '13px', opacity: 0.85, borderTop: '1px solid rgba(255,255,255,0.3)', paddingTop: '12px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                <span>収入管理:</span>
                <span>¥{totalIncomeFromRecords.toLocaleString()}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span>会費収入:</span>
                <span>¥{totalMembershipFees.toLocaleString()}</span>
              </div>
            </div>
          </div>
  ...
  return (
    <>
      ...
      <div className="mobile-header">
        ...
        <h1 style={{ fontSize: '18px', fontWeight: '700', marginLeft: '12px', flex: 1, margin: 0 }}>会計管理</h1>
      </div>
        ...
        <nav style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
          {isAccounting ? (
            <>
              ...
              <button
                onClick={() => handleViewChange('fiscal-years')}
                style={{
                  width: '100%',
                  padding: '12px 16px',
                  marginBottom: '8px',
              ...
              <button
                onClick={() => handleViewChange('membership-fees')}
                style={{
                  width: '100%',
                  padding: '12px 16px',
                  marginBottom: '8px',
              ...
              <button
                onClick={() => handleViewChange('coach-payments')}
                style={{
                  width: '100%',
                  padding: '12px 16px',
                  marginBottom: '8px',
            ...
            <>
              ...
              <button
                onClick={() => handleViewChange('coach-activities')}
                style={{
                  width: '100%',
                  padding: '12px 16px',
                  marginBottom: '8px',
